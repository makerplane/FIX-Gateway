...
class MainThread(threading.Thread):
    def __init__(self, parent):
        ...
        SPI_PORT   = 0
        SPI_DEVICE = 0
        self.CLK = int(parent.config['clk']) if ('clk' in parent.config) and parent.config['clk'] else 18
        self.MISO = int(parent.config['miso']) if ('miso' in parent.config) and parent.config['miso'] else 23
        self.MOSI = int(parent.config['mosi']) if ('mosi' in parent.config) and parent.config['mosi'] else 24
        self.CS = int(parent.config['cs']) if ('cs' in parent.config) and parent.config['cs'] else 25
        self.VKEY1 = parent.config['vkey1'] if ('vkey1' in parent.config) and parent.config['vkey1'] else "ANLG1"
        self.VKEY2 = parent.config['vkey2'] if ('vkey2' in parent.config) and parent.config['vkey2'] else "ANLG2"
        self.VKEY3 = parent.config['vkey3'] if ('vkey3' in parent.config) and parent.config['vkey3'] else "ANLG3"
        self.VKEY4 = parent.config['vkey4'] if ('vkey4' in parent.config) and parent.config['vkey4'] else "ANLG4"
        self.VKEY5 = parent.config['vkey5'] if ('vkey5' in parent.config) and parent.config['vkey5'] else "ANLG5"
