...
class Mav:
    ...
    def request_ids(self):
        for msg_id in self.ids:
            ...
            response = self.conn.recv_match(
                type="COMMAND_ACK", timeout=0.1, blocking=True
            )
            if (
                response
    ...
    def process(self):
        ...
        #                self.parent.db_write("GPS_SATS_TRACKED", msg.satellite_used.count(1))
            ...
            if self._gps:
                ...
                self.parent.db_write(
                    "GPS_ELLIPSOID_ALT", round(msg.alt_ellipsoid / 304.8, 2)
                )  # int32 mm to ft
                self.parent.db_write("GPS_SATS_VISIBLE", msg.satellites_visible)  #
                self.parent.db_write(
                    "GPS_SATS_TRACKED", msg.satellites_visible
                )  # Not found a way to get tracked
                self.parent.db_write(
                    "GPS_ACCURACY_SPEED", round(msg.vel_acc * 0.0019438445, 2)
                )  # int32 mm/s to knots Docs just sa "mm" I assumed/sec
                self.parent.db_write(
                    "GPS_ACCURACY_HORIZ", round(msg.h_acc / 3048, 2)
                )  # / 3.048,2)) # int32 mm to ft
                self.parent.db_write(
                    "GPS_ACCURACY_VERTICAL", round(msg.v_acc / 3048, 2)
                )  # / 3.048,2)) # int mm to ft
            ...
            if self._ahrs:
                ...
                self.parent.db_write(
                    "AGL", round(msg.relative_alt / 304.8, 2)
                )  # int32_t mm to ft
                # Seems like MSD is TALT and ALT should be indicated, not sure hoe to get both:
                self.parent.db_write(
                    "ALT", round(msg.alt / 304.8, 2)
                )  # int32_t mm to ft
                self.parent.db_write(
                    "TALT", round(msg.alt / 304.8, 2)
                )  # int32_t mm to ft
            if self._gps:
                self.parent.db_write("LAT", msg.lat / 10000000.0)  # int32_t degE7 10**7
                self.parent.db_write(
                    "LONG", msg.lon / 10000000.0
                )  # int32_t degE7 10**7
        elif msg_type == "SCALED_PRESSURE":
            if self._pressure:
                self.parent.db_write(
                    "AIRPRESS", round((msg.press_abs * 100) + self._pascal_offset, 4)
                )  # float hPa to Pa
                self.parent.db_write(
                    "DIFFAIRPRESS", round(msg.press_diff * 100, 4)
                )  # float hPa to Pa
        ...
        # imus/gyros
            ...
            if msg.type == 1:
                ...
                if msg.base_mode & mavutil.mavlink.MAV_MODE_FLAG_SAFETY_ARMED != 0:
                    ...
                    self.setStat("ARMED")
                if (
                    msg.base_mode & mavutil.mavlink.MAV_MODE_FLAG_MANUAL_INPUT_ENABLED
                    != 0
                ):
                ...
                if msg.custom_mode == self._apmodes["TRIM"]:
                    ...
                    self.sendMode(
                        "GUIDED", f"Nav to: {self.parent.db_read('WPNAME')[0]}"
                    )
            ...
            self._outputRoll = int(
                (msg.servo1_raw - 1500) * 2.5
            )  # 1100-1900us 1500 center
    ...
    def checkWaypoint(self):
        ...
        if (
            self.parent.db_read("WPLAT")[2]
            or self.parent.db_read("WPLON")[2]
            or (
                self.parent.db_read("WPLAT")[0] == 0.0
                or self.parent.db_read("WPLON")[0] == 0.0
            )
        ):
            ...
            if self._apreq == "GUIDED" and self.parent.quorum.leader:
                ...
                # order matters:
                self.parent.db_write("MAVREQGUIDED", False)
            ...
            if (
                self._waypoint
                != f"{self.parent.db_read('WPLON')[0]}{self.parent.db_read('WPLAT')[0]}{self.parent.db_read('WPNAME')[0]}"
                and self._apmode == "GUIDED"
            ):
                if self.parent.quorum.leader:
                    self.setMode("GUIDED")
    ...
    def setMode(self, mode):
        ...
        if self._apstat in ["TRIM", "ARMED"] and mode in self._apmodes:
            if mode not in ["TRIM"] and (
                (mode == "GUIDED" and not self._apwpv) or (self._apstat != "ARMED")
            ):
            ...
            if mode == "GUIDED":
                ...
                message = self.conn.mav.command_int_encode(
                    ...
                    self.parent.db_read("ALT")[0]
                    * 0.3048,  # altitude ALT is in ft, mavlink uses meters
                )
            else:
                message = self.conn.mav.command_long_encode(
                    self.conn.target_system,
