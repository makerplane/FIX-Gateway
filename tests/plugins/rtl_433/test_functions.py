import pytest
from fixgw.plugins.rtl_433 import convert_type, apply_transform, get_rtl_433_path, start_rtl_433, process_json
import os
import subprocess
...
def test_start_rtl_433(mock_popen):
    ...
    process = start_rtl_433(MagicMock(), simulate=False, device=device, frequency=frequency, decoders=decoders)

    # Ensure Popen was called with the expected arguments
    mock_popen.assert_called_once_with(
        ["rtl_433", "-d", "0", "-f", "433920000", "-F", "json", "-R", "203", "-R", "204"],
...
def test_start_rtl_433_simulate(mock_popen):
    ...
    process = start_rtl_433(MagicMock(), simulate=True, device=0, frequency=433920000, decoders=[])
...
def test_process_json_valid(mock_plugin):
    ...
    mock_plugin.db_write.assert_any_call("TIRE_PRESSURE1", pytest.approx(250 * 0.145032632, 0.1))  # PSI conversion
    mock_plugin.db_write.assert_any_call("TIRE_TEMP1", (30 - 40))  # Temperature offset applied
    mock_plugin.db_write.assert_any_call("TIRE_BATOK1", 1)  # Battery voltage > 2.0 should set to 1 (OK)
